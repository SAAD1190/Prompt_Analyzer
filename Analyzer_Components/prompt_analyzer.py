...
##################################################################################################################
    ...
    def process_prompts(self, sort_by="svr", reference_prompts=None, reverse=True, output_file="processed_prompts.json"):
            ...
            if sort_by == "vr":
                results = self.compute_vocabulary_richness()
            elif sort_by == "sr":
                results = self.compute_semantic_richness()
            elif sort_by == "svr":
                results = self.compute_svr()
            elif sort_by == "lexical_density":
                lexical_densities = self.lexical_density()
                results = [(prompt, score) for prompt, score in zip(prompts, lexical_densities)]
            elif sort_by == "parse_tree_depth":
                parse_tree_depths = self.parse_tree_depth()
                ...
                results = self.relevance(reference_prompts)
            else:
